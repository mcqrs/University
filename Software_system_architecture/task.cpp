...
void Schedule(int task, int mode) {
	...
	// preemptive алгоритм планирования
...
void Dispatch() {
	...
	while (task != _NULL) {
		...
		case TASK_READY:
			...
			printf("%s started\n", TaskQueue[task].name); RunningTask = task;
			...
			return;
		case TASK_WAITING:
			if ((TaskQueue[task].waiting_events & TaskQueue[task].events)==TaskQueue[task].waiting_events) {
				TaskQueue[task].events &= ~TaskQueue[task].waiting_events;
				TaskQueue[task].waiting_events = 0;
				if ((TaskQueue[task].res != _NULL && ResourceQueue[TaskQueue[task].res].task == _NULL) || TaskQueue[task].res == _NULL) {
					TaskQueue[task].state = TASK_READY;
				}
			}
			else {
