/*** Generated by Spin Version 4.3.0 -- 22 June 2007 ***/
/*** From source: pan_in ***/
...
#define Index(x, y)	Boundcheck(x, y, 0, 0, 0)
	...
	case 1:	/* writer */
		((P1 *)pptr(h))->_t = 1;
		((P1 *)pptr(h))->_p = 131; reached1[131]=1;
		/* params: */
		((P1 *)pptr(h))->i = par0;
		/* locals: */
		((P1 *)pptr(h))->countZeros = 0;
		((P1 *)pptr(h))->t = 0;
#ifdef VAR_RANGES
		logval("writer:i", ((P1 *)pptr(h))->i);
		logval("writer:countZeros", ((P1 *)pptr(h))->countZeros);
		logval("writer:t", ((P1 *)pptr(h))->t);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* reader */
		...
		logval("reader:cZeros", ((P0 *)pptr(h))->cZeros);
		logval("reader:p", ((P0 *)pptr(h))->p);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(void)
{
	...
	{	int l_in;
		...
		now.RW.OKtoRead.waiting = 0;
		now.RW.OKtoWrite.gate = 0;
		now.RW.OKtoWrite.waiting = 0;
#ifdef VAR_RANGES
		logval("lock", now.lock);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("wantR[l_in]", now.wantR[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("critR[l_in]", now.critR[l_in]);
		}
	}
		logval("num", now.num);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("waitR[l_in]", now.waitR[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("waitW[l_in]", now.waitW[l_in]);
		}
	}
		logval("RW.readers", now.RW.readers);
		logval("RW.writers", now.RW.writers);
		logval("RW.OKtoRead.gate", now.RW.OKtoRead.gate);
		logval("RW.OKtoRead.waiting", now.RW.OKtoRead.waiting);
		logval("RW.OKtoWrite.gate", now.RW.OKtoWrite.gate);
...
{	/* int i; */
	...
	printf("	int    num:	%d\n", now.num);
		printf("	(struct RW)\n");
	printf("	int    readers:	%d\n", now.RW.readers);
	printf("	int    writers:	%d\n", now.RW.writers);
	printf("	bit    gate:	%d\n", now.RW.OKtoRead.gate);
	printf("	int    waiting:	%d\n", now.RW.OKtoRead.waiting);
	printf("	bit    gate:	%d\n", now.RW.OKtoWrite.gate);
	printf("	int    waiting:	%d\n", now.RW.OKtoWrite.waiting);
	printf("	bit    lock:	%d\n", now.lock);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	bit    wantR[%d]:	%d\n", l_in, now.wantR[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	bit    critR[%d]:	%d\n", l_in, now.critR[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	int    waitR[%d]:	%d\n", l_in, now.waitR[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	int    waitW[%d]:	%d\n", l_in, now.waitW[l_in]);
		}
	}
}
void
...
{	/* int i; */
	switch(tp) {
	case 3:
		printf("local vars proc %d (:never:):\n", pid);
		/* none */
		break;
	case 2:
	...
	case 1:
		...
		printf("local vars proc %d (writer):\n", pid);
	printf("	int    i:	%d\n", ((P1 *)pptr(pid))->i);
	printf("	int    countZeros:	%d\n", ((P1 *)pptr(pid))->countZeros);
	printf("	int    t:	%d\n", ((P1 *)pptr(pid))->t);
		break;
	case 0:
